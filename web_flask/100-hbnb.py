#!/usr/bin/python3
"""This module creates a minimalist flask app"""
from flask import Flask, escape, render_template
from models import storage
from models.state import State
from models.amenity import Amenity
from models.place import Place
from models.user import User
app = Flask(__name__)


@app.route('/hbnb', strict_slashes=False)
def all_states():
    """Returns all states"""
    states = [x for x in storage.all(State).values()]
    amen = [x for x in storage.all(Amenity).values()]
    places = [x for x in storage.all(Place).values()]
    users = [x for x in storage.all(User).values()]
    states.sort(key=lambda x: x.name)
    amen.sort(key=lambda x: x.name)
    places.sort(key=lambda x: x.name)
    return render_template("100-hbnb.html",
                           states=states,
                           amen=amen, places=places, users=users)


@app.teardown_appcontext
def end_session(exception=None):
    """Ends the app"""
    storage.close()


if __name__ == "__main__":
    app.run(host="0.0.0.0", port="5000", debug=True)
